...
use App\Services\YS\PmdRightService;
use Illuminate\Http\Request;

class PmdRightController extends Controller {
	...
	public function index(Request $request)
	{
        ...
        $query      = new PmdRightService();
        $story      = $query->getPmdRightData($version,$integrate,$startTime,$endTime);
        $arr=array();
        if($story){
    ...
    public function getData(Request $request){
        ...
        $startTime  = $request->input('startTime',date('Y-m-d',$time-7*24*60*60));
        $endTime    = $request->input('endTime',date('Y-m-d',$time+24*60*60));
        $PmdRight = PmdrightModel::where('intVersionID','=',$version)
