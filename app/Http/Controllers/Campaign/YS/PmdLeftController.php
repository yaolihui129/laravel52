...
use App\Services\YS\PmdLeftService;
use Illuminate\Http\Request;


class PmdLeftController extends Controller {
	...
	public function index(Request $request)
	{
        ...
        $query      = new PmdLeftService();
        $pmdLeft    = $query->getPmdLeftData($version,$integrate,$startTime,$endTime);
        $arr=array();
        if($pmdLeft){
            foreach ($pmdLeft as $rows) {
                $data[] = $rows;
                $arr['success'] = 1;
                $arr['data'] = $data;
            }
