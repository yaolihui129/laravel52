	...
	return util = {
		...
		sendCheckCode : function() {
			...
			if (!error) {
				...
				util.setCCSending();
				CommonUtil.requestService('/password/sendcheckcode', data,
						true, "post", function(data) {// 验证码已发送出去
							if (data.success) {
								var timeMark = 120;
		...
		valiStep : function() {
			var error = ValidateUtil.validate("resetInfo");
			if (!error) {
				var email = $("#email").val();
		...
		postRegister : function() {
			...
			if (data) {
				CommonUtil.requestService('/password/reset', data, false,
						"post", function(data) {
							if (data.success)
